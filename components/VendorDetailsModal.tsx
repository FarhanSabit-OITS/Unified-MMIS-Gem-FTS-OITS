...
export const VendorDetailsModal: React.FC<VendorDetailsModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[32px] shadow-2xl max-w-2xl w-full overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
        {/* Header */}
                   ...
                   <div className="bg-slate-900 rounded-[24px] overflow-hidden border border-slate-800 shadow-2xl">
                       <table className="w-full text-[11px] text-left text-slate-300">
                           <thead className="bg-slate-800/50 text-slate-400 font-black uppercase tracking-widest border-b border-slate-800">
                               <tr>
                                   <th className="px-5 py-5 text-indigo-400">Registry Ref</th>
                                   <th className="px-5 py-5">Timestamp</th>
                           ...
                           <tbody className="divide-y divide-slate-800 font-medium">
                               {vendorTransactions.map(tx => (
                                   <tr key={tx.id} className="hover:bg-indigo-600/5 transition-colors">
                                       <td className="px-5 py-5 font-mono text-indigo-400 font-bold uppercase tracking-tight">{tx.reference || tx.id}</td>
                                       <td className="px-5 py-5 text-slate-500 font-bold">{tx.date}</td>
                                       <td className="px-5 py-5 font-black text-white text-right">{tx.amount.toLocaleString()}</td>
                               ...
                               {vendorTransactions.length === 0 && (
                                   <tr><td colSpan={4} className="px-5 py-12 text-center text-slate-600 italic font-medium uppercase tracking-widest text-[9px]">No financial telemetry triangulated.</td></tr>
                               )}
                           </tbody>
                       </table>
                   </div>
